...
ssh root@localhost -p 2222 command mysql -uroot -phadoop -e "CREATE DATABASE HW; CREATE TABLE HW.weather (id VARCHAR(15), date DATE, tmin VARCHAR(5), tmax VARCHAR(5), snow VARCHAR(5), snwd VARCHAR(5), rcpc VARCHAR(5));"  
ssh root@localhost -p 2222 command mysql -uroot -phadoop -e "CREATE TABLE HW.staging_weather (id VARCHAR(15), date DATE, tmin VARCHAR(5), tmax VARCHAR(5), snow VARCHAR(5), snwd VARCHAR(5), rcpc VARCHAR(5));"
ssh root@localhost -p 2222 command "sqoop export --connect jdbc:mysql://localhost/HW --table weather --username root --password hadoop --export-dir /user/raj_ops/weather -m 8 --input-fields-terminated-by ',' --lines-terminated-by '\n' --staging-table staging_weather --ñlear-staging-table"
ssh root@localhost -p 2222 command mysql -uroot -phadoop -e "DROP TABLE HW.staging_weather;"
